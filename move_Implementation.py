...
def resigned():
    ...
    globals.resign_draw_socket_recieving.sendall(resultant_string.encode())
...
def wait_for_resign_or_draw_event():
    while True:
        try:
            temp_str = globals.resign_draw_socket_recieving.recv(1024).decode()
            ...
            if (globals.is_client): globals.resign_draw_socket_recieving.connect((globals.other_ip_address,globals.server_port))
            else : globals.resign_draw_socket_recieving,_ = globals.my_server_socket.accept()
        ...
        if (temp_str == "Resigning"):
            ...
            print("String got ==== gadbad " + temp_str)
...
def others_move():
    ...
    if (not globals.game_ended):
        ...
        try:
            str_other = globals.game_socket.recv(1024).decode()
            if (len(str_other) == 1) :
                if (str_other == '1') :
                    initiate_resign()
                elif (str_other == "0") :
        ...
        if yuci1 in globals.board.legal_moves:
            ...
            if (prom_char == 't'):
                ...
                if globals.newp=='q' and globals.color_val:
                        assign_new_piece(globals.button_list[prev],"Image_piece/wq.png")
                elif globals.newp=='r' and globals.color_val:
                        assign_new_piece(globals.button_list[prev],"Image_piece/wr.png")
                elif globals.newp=='b' and globals.color_val:
                        assign_new_piece(globals.button_list[prev],"Image_piece/wb.png")
                elif globals.newp=='n' and globals.color_val:
                        assign_new_piece(globals.button_list[prev],"Image_piece/wn.png")
                elif globals.newp=='q' and not globals.color_val:
                        assign_new_piece(globals.button_list[prev],"Image_piece/bq.png")
                elif globals.newp=='r' and not globals.color_val:
                        assign_new_piece(globals.button_list[prev],"Image_piece/br.png")
                elif globals.newp=='b' and not globals.color_val:
                        assign_new_piece(globals.button_list[prev],"Image_piece/bb.png")
                elif globals.newp=='n' and not globals.color_val:
                        assign_new_piece(globals.button_list[prev],"Image_piece/bn.png")
                uci += globals.newp
            else:
                uci += prom_char
