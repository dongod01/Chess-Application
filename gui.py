...
def send_move(prev,k):
    ...
    main.send_move_string(resultant_string)

def others_move():
    str_other = main.recieve_move_string()
    prev,k = int(str_other.split())
...
def my_move(k):
    ...
    if x:
        if (k in chess_list):
            if ((val and board.turn == True) or not(val or board.turn == False)):
                prev = k
                print(k)
